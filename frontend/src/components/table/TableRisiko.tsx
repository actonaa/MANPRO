...
export default function TableRisiko({
  ...
  return (
    <div>
      {/* 📱 MOBILE & TABLET → CARD (1 / 2 columns) */}
      <div className="lg:hidden mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          paginatedData.map((item) => (
            ...
            >
              <div className="flex justify-between mb-2">
                <p className="text-sm text-gray-500">{item.id.slice(0, 8).toUpperCase()}</p>
                <a
                  href={`/risiko/${item.id}`}
        ...
        {filteredData.length > ITEMS_PER_PAGE && (
          <div className="flex justify-center items-center gap-2 py-5">
            <button
              ...
              className={`px-2 text-lg ${currentPage === 1 ? "text-gray-400 cursor-not-allowed" : "text-gray-700 hover:text-blue-600"}`}
            >
              ‹
            </button>
              ...
              return pages.map((p, i) =>
                p === "…" ? (
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-md flex items-center justify-center text-sm transition ${currentPage === p ? "bg-gray-200 text-black font-semibold" : "text-gray-700 hover:text-blue-600"}`}
                  >
                    {p}
                  </button>
                )
            ...
            <button
              ...
              className={`px-2 text-lg ${currentPage === totalPages ? "text-gray-400 cursor-not-allowed" : "text-gray-700 hover:text-blue-600"}`}
            >
              ›
            </button>
          </div>
        ...
        <div className="text-sm p-2 text-gray-600 mt-3">
          Menampilkan <span className="font-semibold">{filteredData.length === 0 ? 0 : startIndex + 1}</span>–
          <span className="font-semibold">{Math.min(endIndex, filteredData.length)}</span> dari <span className="font-semibold">{filteredData.length}</span> data
        </div>
      ...
      <div className="hidden lg:block mt-5 md:mt-0">
        <table className="w-full min-w-[950px] text-[13px] text-center border-collapse">
          ...
          <tbody>
            {paginatedData.length > 0 ? (
              paginatedData.map((item) => (
                <tr key={item.id} className="border-b border-b-[#ddd] hover:bg-gray-50 transition">
                  ...
                  <td className="py-5 px-4">
                    <span className={`px-5 py-2 rounded-[16px] text-sm font-medium ${getLevelColor(item.criteria)}`}>
                      {item.criteria}
                    </span>
                  </td>
                  <td className="py-5 px-4">
                    <span className={`px-5 py-2 rounded-[16px] text-sm font-medium ${getStatusColor(item.status)}`}>
                      {item.status}
                    </span>
                  </td>
                  ...
                  <td className="py-5 px-4">
                    <a href={`/risiko/${item.id}`} className="text-[#0095E8] font-medium hover:underline">
                      Detail
                    </a>
                  </td>
                </tr>
